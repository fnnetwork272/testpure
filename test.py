...
def mass_check_function(message, cards, check_type="mass"):
    ...
    status_message = bot.send_message(
        chat_id,
        "⏳ Checking your cards, please wait...",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton(f"Charged 🔥: 0", callback_data="status_charged"),
             InlineKeyboardButton(f"Declined❌: 0", callback_data="status_declined")],
            [InlineKeyboardButton(f"Total💳: 0/{total}", callback_data="status_total"),
    ...
    for i, card in enumerate(cards):
        ...
        if (i + 1) % 5 == 0 or (i + 1) == total:
            try:
                bot.edit_message_text(
                    f"⏳ Checking your cards, please wait...\nProgress: {i+1}/{total}",
                    chat_id=chat_id,
                    message_id=status_message.message_id,
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton(f"Charged 🔥: {charged}", callback_data="status_charged"),
                         InlineKeyboardButton(f"Declined❌: {declined}", callback_data="status_declined")],
                        [InlineKeyboardButton(f"Total💳: {i+1}/{total}", callback_data="status_total"),
                         InlineKeyboardButton("Stop 🛑", callback_data="stop_check")]
                    ])
                )
            except:
    ...
    try:
        bot.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton(f"Charged 🔥: {charged}", callback_data="status_charged"),
                 InlineKeyboardButton(f"Declined❌: {declined}", callback_data="status_declined")],
                [InlineKeyboardButton(f"Total💳: {total}/{total}", callback_data="status_total")]
            ])
        )
    except:
        pass
    ...
    if charged_cards and check_type == "stxt":
        ...
        bot.send_document(chat_id, bio, caption=f"✅ {charged} charged cards found!")
...
def stxt_check(message):
    ...
    if message.document:
        ...
        if len(cards) > 200:
            bot.reply_to(message, "❌ File check limit is 200 cards.")
            return
...
def stop_check(message):
    ...
    bot.reply_to(message, "🛑 Stop signal sent. The check will stop after the current card.")
