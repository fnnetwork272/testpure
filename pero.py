...
def sh(card_details, username):
    ...
    # BIN Lookup functionality has been removed.
    ...
    result = {
        ...
        'username': username, 'dev': 'D E A T H - 👑'
    }
    return result
...
def mass_check(message):
    ...
    if len(cards) > 200:
        bot.reply_to(message, "❌ Mass check limit is 200 cards.")
        return
    ...
    # Create a list of status strings for each card, initially showing them as pending.
    status_lines = []
    for card in cards:
        # Clean up card format for display (e.g., 515676...768 -> 515676...|..|..|768)
        clean_card = '|'.join(re.findall(r'\d+', card))
        status_lines.append(f"CC: `{clean_card}`\nStatus: ⏳\nResponse: Checking...")

    # Join lines to form the initial message body.
    initial_message_text = f"**Shopify Mass Check**\nLimit: {total_cards}/200\n" + ("-"*25) + "\n\n"
    initial_message_text += "\n\n".join(status_lines)

    # Send the initial message that we will be editing.
    sent_msg = bot.reply_to(message, initial_message_text, parse_mode="Markdown")
    
    current_message_text = initial_message_text

    # --- Loop, Check, and Edit ---
    for i, card in enumerate(cards):
        # Call the main checker function
        result = sh(card, username)

        # Format the result string for the current card
        if isinstance(result, str):  # This means sh() returned an error string
            clean_card = '|'.join(re.findall(r'\d+', card))
            result_text = f"CC: `{clean_card}`\nStatus: ❌ Error\nResponse: {result}"
        else:  # This means sh() returned a result dictionary
            result_text = f"CC: `{result['full_card']}`\nStatus: {result['status']}\nResponse: {result['resp_msg']}"

        # Update the specific line in our list of statuses
        status_lines[i] = result_text

        # Rebuild the message text with the updated information
        updated_message_text = f"**Shopify Mass Check**\nLimit: {total_cards}/200\n" + ("-"*25) + "\n\n"
        updated_message_text += "\n\n".join(status_lines)
        
        # Edit the message, but only if the content has changed to avoid API errors
        if updated_message_text != current_message_text:
            try:
                ...
                current_message_text = updated_message_text
            except telebot.apihelper.ApiTelegramException as e:
                # Ignore "message is not modified" error, print others
                if 'message is not modified' not in str(e):
                    print(f"Error editing mass check message: {e}")
        
        # Increased delay to prevent hitting Telegram's API rate limits and Shopify rate limits
        time.sleep(3)

    # Send a final confirmation message after the loop is complete
    bot.send_message(message.chat.id, f"✅ Mass check finished for {total_cards} cards.", reply_to_message_id=message.message_id)
